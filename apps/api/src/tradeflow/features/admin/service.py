        ...
        for conn in rows.all():
            ...
            results.append(
                AdminBrokerStatusResponse(
                    ...
                    broker=_enum_label(conn.broker),
                    name=conn.name,
                    status=conn.status,
        ...
        for ticket in tickets.all():
            results.append(
                AdminSearchResult(
                    ...
                    subtitle=_enum_label(ticket.status),
                ),
            )
        ...
        *,
        ...
        return [
            AdminTradingAccountResponse(
                ...
                account_type=_enum_label(row.account_type),
                account_role=_enum_label(row.account_role),
                status=_enum_label(row.status),
...
def _user_response(user: User) -> AdminUserResponse:
    return AdminUserResponse(
        ...
        roles=[_enum_label(ur.role.name) for ur in user.user_roles],
